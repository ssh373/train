...
def kick_ball_target_direction_b(
    ...
    """Return the unit direction from ball to target in robot yaw frame.

    The two-value contract is kept identical to ``kick_target_pos_b`` so the
    policy observation dimension and the existing actor checkpoint remain
    unchanged.  Distance is deliberately discarded; the direction is defined
    from the ball because that is the quantity the kick must control.
    """
    robot: Articulation = env.scene[robot_cfg.name]
    ball: RigidObject = env.scene[ball_cfg.name]
