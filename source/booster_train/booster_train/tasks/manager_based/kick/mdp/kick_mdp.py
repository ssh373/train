...
def camera_ball_pos_b(env: ManagerBasedEnv, ball_cfg: SceneEntityCfg = SceneEntityCfg("ball")) -> torch.Tensor:
    return _camera_ball_observation(env, ball_cfg)[0]
...
def reset_kick_target(
    ...
    feet_pos_w = robot.data.body_pos_w[env_ids][:, feet_cfg.body_ids]
...
def kicking_foot_approach_ball(
    ...
    return closing_speed * ((1.0 - velocity_weight) * proximity_gate + velocity_weight) * stationary
